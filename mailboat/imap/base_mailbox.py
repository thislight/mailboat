...
MailboxDataT_co = TypeVar(
    "MailboxDataT_co", bound="MailboxDataInterface", covariant=True
)
...
class MailboxDataInterface(Protocol[MessageT_co]):
    ...
    @abstractmethod
    async def copy(
        self: MailboxDataT, uid: int, destination: MailboxDataT, *, recent: bool = False
    ) -> Optional[int]:
    ...
    async def find(
        self, seq_set: SequenceSet, selected: SelectedMailbox
    ...
    async def find_deleted(
        ...
        return [
            msg.uid
            async for _, msg in self.find(seq_set, selected)
