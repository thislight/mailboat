# Copyright(c) 2021 The Mailboat Contributors, icgood
...
from mailboat.utils.refcount import Ref
from mailboat.usrsys.mailbox import MailBox
...
from .base_mailbox import MailboxDataInterface, MailboxSetInterface
...
class _ModSequenceMapping:
    ...
    def _remove_prev(
        self, uid: int, prev_mod_seq: int, data: dict[int, set[int]]
    ) -> None:
    ...
    def find_updated(self, mod_seq: int) -> tuple[frozenset[int], frozenset[int]]:
...
class _ContentCache:
    def __init__(self) -> None:
        ...
        self._content: WeakValueDictionary[
            ObjectId, MessageContent
        ] = WeakValueDictionary()
...
class _ThreadCache:
    def __init__(self) -> None:
        super().__init__()
        self._thread_ids: WeakKeyDictionary[ThreadKey, ObjectId] = WeakKeyDictionary()
        self._ref: WeakKeyDictionary[
            MessageContent, Sequence[ThreadKey]
        ] = WeakKeyDictionary()
...
class MailboxData(MailboxDataInterface[Message]):
    ...
    LOCKS: Dict[str, Ref[ReadWriteLock]] = {}

    @classmethod
    def get_lock(cls, mailbox: MailBox) -> ReadWriteLock:
        lock = cls.LOCKS.get(mailbox.id)
        if not lock:
            lock = Ref(subsystem.get().new_rwlock())
            cls.LOCKS[mailbox.id] = lock
        return lock.ref()

    @classmethod
    def drop_lock(cls, mailbox: MailBox) -> None:
        lock = cls.LOCKS.get(mailbox.id)
        if lock:
            lock.unref()
            if lock.refc <= 0:
                cls.LOCKS.pop(mailbox.id)

    def __init__(
        self, content_cache: _ContentCache, thread_cache: _ThreadCache, mailbox: MailBox
    ) -> None:
    ...
    def mailbox_id(self) -> ObjectId:
        return ObjectId(self._mailbox.id.encode("ascii"))

    @property
    def readonly(self) -> bool:
        return self._mailbox.readonly
    ...
    def messages_lock(self) -> ReadWriteLock:
        return self.get_lock(self._mailbox.id)
    ...
    async def update_selected(
        self, selected: SelectedMailbox, *, wait_on: Event = None
    ) -> SelectedMailbox:
        ...
        if mod_sequence is None:
            ...
            updated_messages = [
                self._messages[uid] for uid in updated if uid in self._messages
            ]
...
class MailboxSet(MailboxSetInterface[MailboxData]):
    ...
    async def list_subscribed(self) -> ListTree:
        async with self._set_lock.read_lock():
            mailboxes = [
                child for child in self._set.keys() if self._subscribed.get(child)
            ]
    ...
    async def add_mailbox(self, name: str) -> ObjectId:
        ...
        async with self._set_lock.write_lock():
            self._set[name] = mbx = MailboxData(self._content_cache, self._thread_cache)
