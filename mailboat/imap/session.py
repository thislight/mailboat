...
from pymap.parsing.specials import SequenceSet, SearchKey, ObjectId, FetchRequirement
...
class BaseSession(SessionInterface, Generic[MessageT]):
    ...
    @property
    @abstractmethod
    def mailbox_set(self) -> MailboxSetInterface[MailboxDataInterface[MessageT]]:
        ...
    ...
    async def _load_updates(
        self,
        selected: Optional[SelectedMailbox],
    ...
    @classmethod
    def _pick_selected(
        cls, selected: Optional[SelectedMailbox], mbx: MailboxDataInterface[MessageT]
    ) -> Optional[SelectedMailbox]:
    ...
    async def _get_mailbox(
        self, name: str, *, try_create: bool = False
    ...
    async def list_mailboxes(
        ...
        Iterable[tuple[str, Optional[str], Sequence[bytes]]], Optional[SelectedMailbox]
    ]:
        ...
        if filter_:
            ...
            ret = [
                (entry.name, delimiter, entry.attributes)
    ...
    async def get_mailbox(
        self, name: str, selected: SelectedMailbox = None
    ...
    async def rename_mailbox(
        self, before_name: str, after_name: str, selected: SelectedMailbox = None
    ) -> Optional[SelectedMailbox]:
        try:
    ...
    async def append_messages(
        ...
        for append_msg in messages:
            ...
            uids.append(msg.uid)
        return (
            AppendUid(mbx.uid_validity, uids),
    ...
    async def fetch_messages(
        self, selected: SelectedMailbox, sequence_set: SequenceSet, set_seen: bool
        ...
        for seq, cached_msg in selected.messages.get_all(sequence_set):
            if set_seen:
                msg = await mbx.update(
                    cached_msg.uid, cached_msg, frozenset({Seen}), FlagOp.ADD
                )
            else:
    ...
    async def search_mailbox(
        self, selected: SelectedMailbox, keys: frozenset[SearchKey]
    ) -> tuple[Iterable[tuple[int, MessageT]], SelectedMailbox]:
        mbx = await self._get_selected(selected)
    ...
    async def expunge_mailbox(
        self, selected: SelectedMailbox, uid_set: SequenceSet = None
    ) -> SelectedMailbox:
    ...
    async def copy_messages(
        self, selected: SelectedMailbox, sequence_set: SequenceSet, mailbox: str
        ...
        for _, source_uid in selected.messages.get_uids(sequence_set):
            dest_uid = await mbx.copy(source_uid, dest, recent=not dest_selected)
    ...
    async def move_messages(
        ...
        uids: list[tuple[int, int]] = []
        for _, source_uid in selected.messages.get_uids(sequence_set):
            dest_uid = await mbx.move(source_uid, dest, recent=not dest_selected)
            if dest_uid is not None:
                if dest_selected:
                    dest_selected.session_flags.add_recent(dest_uid)
                uids.append((source_uid, dest_uid))
