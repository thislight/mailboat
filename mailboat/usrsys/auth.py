...
class AuthProvider(object):
    def __init__(self, user_record_storage: UserRecordStorage, token_record_storage: TokenRecordStorage) -> None:
        ...
        if request.username and request.password:
            ...
            if password_checking:
                ...
                if request.request_token:
                    user_record = await self.user_record_storage.find_one({'username': request.username})
                    if user_record:
                        token = await self.token_record_storage.create_token(user_record.profileid, appid=request.appid, scope=request.new_token_scope) # TODO (rubicon): set apprev from latest app config (after app things done)
                return AuthAnswer(handled=True, success=True, required_second_factors=[], scope=[], token=(token.token if token else None))
            else:
